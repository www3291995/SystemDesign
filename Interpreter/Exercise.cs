...
namespace DesignPattern.Interpreter
{
    ...
    public class TestE
    {
        ...
        {
            var ex = new Exercise();
            ex.Variables.Add('x', 5);

            Assert.That(ex.Calculate("1"), Is.EqualTo(1));
            Assert.That(ex.Calculate("1+2+3"), Is.EqualTo(6));
            Assert.That(ex.Calculate("1+2+xy"), Is.EqualTo(0));
            Assert.That(ex.Calculate("10-2-x"), Is.EqualTo(3));
        }
    }
}
